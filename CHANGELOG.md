...
- Every pipeline stage execution now runs as a durable agent thread: an append-only record log in Postgres carrying the rendered prompt as sent and the model response with its usage, committed in the same transactions as the work itself. Stage behaviour, configuration, the CLI, the MCP schemas, and `tribal_job_status` semantics are unchanged; durability and recording are additions underneath. Inference spend gains thread and record attribution in the `token_usage` ledger.
