...
/// Exercises the pre-insert endpoint validation safety net.
///
/// This fabricates an edge case that cannot occur in normal operation:
/// knowledge items are never deleted, and the graph is append-only.
/// The test manually deletes one item between seeding and running the
/// relation stage to verify that the validation drops the affected
/// edge rather than letting the FK constraint fail the entire batch.
#[tokio::test]
async fn test_relation_stage_drops_edges_with_deleted_endpoint() {
    let _guard = serial_lock().await;
    let ctx = test_context().await;
    let pool = ctx.create_pool().await.expect("create pool");

    let (principal_id, project_id, system_pv_id, user_pv_id) =
        setup_prerequisites(ctx, "relation-deleted-endpoint").await;

    let candidates = vec![
        a_candidate()
            .content("First candidate".to_owned())
            .build(),
        a_candidate()
            .content("Second candidate".to_owned())
            .build(),
    ];
    let relation_hints = vec![a_relation_hint().build()];
    ...
    // Fabricate the edge case: delete one knowledge item so its ID
    // no longer exists when the relation commit validates endpoints.
    {
        let mut conn = raw_conn(ctx).await;
        let deleted = PgKnowledgeItemRepository
            .delete_by_id_for_test(&mut conn, ki_ids[1])
            .await
            .expect("delete knowledge item for test");
        assert!(deleted, "item should have existed before deletion");
    }

    // The mock returns an edge from item 0 → item 1. Item 1 no
    // longer exists, so the validation should drop this edge.
    let inference: Arc<dyn InferenceProvider> = Arc::new(
        MockInferenceProvider::builder()
            .on_complete(
                a_completion_response(context_index_relation_response_json(ki_ids.len())),
    ...
    assert_eq!(
        job.outcome(),
        Some(JobOutcome::Success),
        "job should complete successfully despite dropped edge",
    );

    // The edge targeting the deleted item should have been dropped
    // by pre-insert validation, not by an FK failure.
    let mut conn = raw_conn(ctx).await;
    let outbound = PgRelationRepository
        .find_outbound(&mut conn, ki_ids[0], None)
        .await
    ...
    assert!(
        outbound.is_empty(),
        "edge to deleted item should be dropped, not committed",
    );
