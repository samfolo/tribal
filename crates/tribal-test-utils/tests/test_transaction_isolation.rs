...
async fn test_transaction_rolls_back_on_drop() {
    ...
    // Insert a row inside a TestTransaction, then drop it.
    {
        ...
        let count: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM principals WHERE principal_key = 'rollback-test'",
        )
        .fetch_one(&mut *txn)
        .await
    ...
    assert_eq!(count.0, 0, "row should not exist after transaction rollback");
}
...
async fn test_transaction_isolates_concurrent_tests() {
    ...
    let mut txn_a = ctx
        .begin_test()
        .await
        .expect("should begin transaction A");
    let mut txn_b = ctx
        .begin_test()
        .await
        .expect("should begin transaction B");
    ...
    let count: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM principals WHERE principal_key = 'isolation-a'",
    )
    .fetch_one(&mut *txn_b)
    .await
